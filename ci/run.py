...
parser.add_argument('--configure', action = 'store_true', help = 'Whether CMake configuration should be run')
parser.add_argument('--build', action = 'store_true', help = 'Whether the output should be built')
parser.add_argument('--test', action = 'store_true', help = 'Whether the tests should be run')
...
if args.test:
    # Run tests
    command = ["cmake", "--build", ".", "--config", args.config, "--target", "AllTests"]
    print 'Execute: {}'.format(command)
    subprocess.check_call(command)
